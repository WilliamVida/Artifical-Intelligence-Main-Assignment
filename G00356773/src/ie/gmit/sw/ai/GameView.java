...
public class GameView extends Canvas {
	...
	private Color[] reds = { Color.SALMON, Color.CRIMSON, Color.RED }; // Animate enemy "dots" to make them easier to
																		// see
	...
	public void draw() {
		...
		for (int row = 0; row < cellspan; row++) {
			for (int col = 0; col < cellspan; col++) {
				...
				if (zoom) {
					if (ch > '0') {
						g.setFill(row == currentRow && col == currentCol ? PLAYER_COLOUR
								: reds[rand.nextInt(reds.length)]);
